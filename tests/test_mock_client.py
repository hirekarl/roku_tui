...
async def test_keypress_fires_post_callback():
...
async def test_keypress_response_time_set():
    client, events = _make_client()
...
async def test_query_apps_fires_get_callback():
    client, events = _make_client()
...
async def test_query_device_info_fires_callback():
    client, events = _make_client()
...
async def test_query_active_app_fires_callback():
    client, events = _make_client()
...
async def test_launch_fires_post_callback():
    client, events = _make_client()
...
async def test_multiple_calls_fire_separate_events():
    client, events = _make_client()
