...
def test_parse_count_digit():
    assert _parse_count(["3"]) == 3


def test_parse_count_non_digit_defaults_to_one():
    assert _parse_count(["up"]) == 1


def test_parse_count_empty_args_defaults_to_one():
    assert _parse_count([]) == 1


def test_parse_count_capped_at_max():
    assert _parse_count(["999"]) == 30


def test_parse_count_at_offset():
...
async def test_volume_up_sends_keypress():
...
async def test_volume_invalid_direction_returns_usage():
    client, _ = _make_client()
    result = await handle_volume(client, ["sideways"], MockContext())
    assert "Usage" in result


async def test_volume_no_args_returns_usage():
    client, _ = _make_client()
    result = await handle_volume(client, [], MockContext())
    assert "Usage" in result


async def test_volume_with_count():
    client, events = _make_client()
...
async def test_launch_no_args_returns_usage():
    client, _ = _make_client()
    result = await handle_launch(client, [], MockContext())
    assert "Usage" in result


async def test_launch_exact_match():
    client, events = _make_client()
    ctx = MockContext()
...
async def test_launch_no_match_returns_error():
    client, _ = _make_client()
    ctx = MockContext()
    ctx.app_cache = list(MOCK_APPS)
...
async def test_launch_fetches_apps_when_cache_empty():
    client, events = _make_client()
    ctx = MockContext()
    result = await handle_launch(client, ["netflix"], ctx)
...
async def test_connect_no_args_returns_usage():
    client, _ = _make_client()
...
def test_register_all_includes_nav_commands():
    reg = CommandRegistry()
    register_all(reg)
...
def test_register_all_includes_single_letter_aliases():
    reg = CommandRegistry()
    register_all(reg)
...
def test_register_all_volume_alias():
    reg = CommandRegistry()
    register_all(reg)
    assert reg.lookup("vol") is not None
    assert reg.lookup("vol").name == "volume"
